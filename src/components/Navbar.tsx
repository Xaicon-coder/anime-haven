            ...
            <div className="relative flex items-center gap-2 animate-slide-in">
              ...
              {query.length >= 2 && (
                <div className="absolute top-full right-0 mt-2 w-[calc(100vw-2rem)] sm:w-80 bg-card border border-border rounded-xl shadow-2xl overflow-hidden max-h-80 sm:max-h-96 overflow-y-auto z-50">
                  ...
                  {results.map((anime) => (
                    <button key={anime.id} onClick={() => { navigate(`/anime/${anime.id}`); setSearchOpen(false); setQuery(""); }} className="flex items-center gap-3 w-full p-2.5 sm:p-3 hover:bg-secondary transition-colors text-left">
                      <div className="w-8 h-12 sm:w-10 sm:h-14 flex-shrink-0 rounded overflow-hidden bg-secondary"><img src={anime.cover} alt={anime.title} className="w-full h-full object-cover" /></div>
